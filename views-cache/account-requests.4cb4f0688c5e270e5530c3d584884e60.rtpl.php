<?php if(!class_exists('Rain\Tpl')){exit;}?>	<section class="ct-center">
		...
		<div class="ct-ini mt-mobNavbar">
			...
			<nav class="d-md-none my-2">
				<a href="/loja-<?php echo htmlspecialchars( $ID, ENT_COMPAT, 'UTF-8', FALSE ); ?>/" class="btn btn-sm btn-main-site-section text-btn-site-section"><i class="fas fa-chevron-left"></i> <i>Voltar</i></a>
			</nav>
							...
							<table class="table table-hover font-weight-normal h6 text-truncate bg-white border">
								<thead>
								  <tr class="text-center">
									  <th class="border-right text-second-site-section">#</th>
								...
								<tbody>
									...
									<tr class="cursorPointer text-center" onclick="window.location.assign('/loja-<?php echo htmlspecialchars( $ID, ENT_COMPAT, 'UTF-8', FALSE ); ?>/account/requests/<?php echo htmlspecialchars( $value1["idOrder"], ENT_COMPAT, 'UTF-8', FALSE ); ?>/')">
										<th scope="row" class="py-2">
											<a class="text-decoration-none text-second-site-section"><i>#<?php echo htmlspecialchars( $value1["idOrder"], ENT_COMPAT, 'UTF-8', FALSE ); ?>-<?php echo htmlspecialchars( $key1+1, ENT_COMPAT, 'UTF-8', FALSE ); ?></i></a>
										</th>
										<th scope="row" class="py-2">
											<a class="font-weight-normal text-second-site-section text-decoration-none">
										...
										<td class="py-2">
											<p class="text-decoration-none text-second-site-section">
												<span>
													<?php echo utf8_encode(ucwords(strftime('%d ', strtotime($value1["dateHorary"])))); ?> de 
													<?php echo utf8_encode(ucwords(strftime('%B', strtotime($value1["dateHorary"])))); ?> de 
													<?php echo utf8_encode(ucwords(strftime('%Y', strtotime($value1["dateHorary"])))); ?>
												</span>
